...
def advantage_actor_critic(env, estimator_policy, estimator_value, svpg, num_episodes, discount_factor=1.0):
    """
    Advantage Actor Critic Algorithm. Optimizes the policy 
            ...
            if(((t+1)%UPDATE_ITER==0)or(Done)):
              ...
              for i in range(n_particles):
                ...
                for t in range(len(episode[i])):
                  ...
                  if done[i]:
                    buffer_td_target[i].append(sum(discount_factor**idx * trans.reward for idx, trans in enumerate(episode[i][t:])));
                  else:
                    value_next=estimator_value[i].predict(transition.next_state);
                    buffer_td_target[i].append(sum(discount_factor**idx * trans.reward for idx, trans in enumerate(episode[i][t:]))+discount_factor**len(episode[i][t:])*value_next);
                  buffer_td_error[i].append(buffer_td_target[i][-1]-estimator_value[i].predict(buffer_s[i][-1]));
                for st_idx in range(len(buffer_s[i])):
                  buffer_s[i][st_idx]=featurize_state(buffer_s[i][st_idx]);
                estimator_value[i].update(buffer_s[i],buffer_td_target[i]);
                feed_dict.update({estimator_policy[i].state:buffer_s[i]});
                feed_dict.update({estimator_policy[i].target:buffer_td_error[i]});
                feed_dict.update({estimator_policy[i].action:np.reshape(buffer_a[i],[1,-1])});
              svpg.run(feed_dict);
...
with tf.Session() as sess:
    ...
    stats = advantage_actor_critic(env, policy_estimator, value_estimator, svpg, NUM_EPISODES, discount_factor=0.95)
