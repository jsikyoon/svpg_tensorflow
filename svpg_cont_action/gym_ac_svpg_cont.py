    ...
    for i_episode in range(num_episodes):
        ...
        for t in range(MAX_EPI_STEP):
            ...
            for i in range(n_particles):
              ...
              if(ENV_NAME=="Pendulum-v0"):
                td_target = (reward[i]+8)/8 + discount_factor * value_next
              else:
                td_target = reward[i] + discount_factor * value_next
              td_error = td_target - estimator_value[i].predict(state[i])

              # Update the value estimator
              estimator_value[i].update([state[i]], np.reshape(td_target,[-1,1]))
              ...
              if(ENV_NAME=="MountainCarContinuous-v0"):
                ...
                feed_dict.update({estimator_policy[i].state:[state[i]]});
              feed_dict.update({estimator_policy[i].target:np.reshape(td_error,[-1,1])});
              feed_dict.update({estimator_policy[i].a_his:np.reshape(action[i],[-1,num_action])});
